...
def update_weights(weights_before, model_after, model_info):
    ...
    for i_layer, (layer_name, layer_info) in enumerate(model_info.items()):
        ...
        if i_layer == 0:
            ...
            # extract infomation of previous layer with the most number of channels.
            len_prev = 0
            i_prev = 0
            for i, prev_key_name in enumerate(layer_info['prev']):
                len_tmp = len(model_info[prev_key_name]['topk'])
                if len_tmp > len_prev:
                    len_prev = len_tmp
                    i_prev = i
            prev_layer_info = model_info[layer_info['prev'][i_prev]]
            prev_idxs = np.sort(prev_layer_info['topk'])        # prev_idxs: index of output channel for previous layer = # of input channels for current layer
            prev_layer_type = prev_layer_info['type']

            if i_layer == len(model_info) - 1:
                # For the last layer
                current_idxs = np.arange(weight_shape_after[0]) # current_idxs: index of output channel for previous layer = # of input channels for current layer
            else:
        ...
        if current_layer_type == 'fc' and prev_layer_type in ('conv', 'bn'):
            # For Linear layers (The previous layers are Conv2d and Conv1d)
            # extract infomation of previous layer with the most number of channels.
            len_prev = 0
            i_prev = 0
            for i, prev_key_name in enumerate(layer_info['prev']):
                ...
                if len_tmp > len_prev:
                    len_prev = len_tmp
                    i_prev = i
            prev_layer_name = layer_info['prev'][i_prev]
            prev_wt_key = prev_layer_name + '.weight' # number of input nodes for fc layer
            ...
            reshaped_weights = weights_before[wt_key].data.clone().reshape(
                [weights_before[wt_key].data.shape[0],
                 int(weights_before[wt_key].numel()/weights_before[wt_key].data.shape[0]), -1])
 
            # Copy weights to keep and reshape
            weights_after[wt_key] = reshaped_weights[idxs_wt_to_keep].reshape(weight_shape_after)
            # Copy bias
            if bs_key in weights_before:
                weights_after[bs_key] = weights_before[bs_key].data.clone()[idxs_bias_to_keep]
        ...
        # For BatchNorm2d and BatchNorm1d layer
            ...
            for k in (wt_key, bs_key, rm_key, rv_key):
                if k in weights_before:
                    weights_after[k] = weights_before[k].data.clone()[idxs_to_keep]
            # Copy bias
            if nb_key in weights_before:
                weights_before[nb_key] = weights_before[nb_key].data.clone()

        # For Conv2d, Conv1d layers, and Linear layer when the previous layers are not Conv2d and Conv1d.
        else:
            # Copy weight
            weights_after[wt_key] = weights_before[wt_key].data.clone()[idxs_wt_to_keep]
            # Copy bias
            if bs_key in weights_before:
                weights_after[bs_key] = weights_before[bs_key].data.clone()[idxs_bias_to_keep]
...
def auto_prune(model_class, model_info, weights_before, acc_before,
               ...
               loss_margin=0.1, acc_control=1.0,
    ...
    # save model
    if use_DataParallel:
        torch.save(model_before.module.state_dict(), pruned_model_path)
    else:
        torch.save(model_before.state_dict(), pruned_model_path)
    ...
    for search_times in range(max_search_times):
        ...
        # Copy weights from the model before pruning to the temporary pruned model
        ...
        for epoch in range(epochs):
            ...
            train(train_loader, model_after, criterion, optimizer, scheduler, lr_scheduler, epoch, args)

            # Validation with re-trained model
            acc_after = validate(val_loader, model_after, criterion, args)
        ...
        ##### update model ###
        if update_model:
            n_args_channels_fix = n_args_channels
            if use_DataParallel:
    ...
    return weights_before, final_acc, n_args_channels_fix


def train(train_loader, model, criterion, optimizer, scheduler, lr_scheduler, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # change learning rate every iteration by LR scueduler
        if lr_scheduler and args.scheduler_timing == 'iter':
            scheduler.step()
    ...
    # change learning rate every epoch by LR scueduler
    if lr_scheduler and args.scheduler_timing == 'epoch':
        scheduler.step()
